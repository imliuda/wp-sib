...
/**
 * sib menu walker
 * Supports separators as "separator" arg to wp_nav_menu call
 */
class SIBMenuWalker extends Walker_Nav_Menu {
    private $counter = 0;
 
    function start_el(&$output, $item, $depth, $args) {
 
        if($this->counter && isset($args->separator))
...
/*
 * use custom post excerpt length.
 */
function sib_excerpt_length($length) {
	return 140;
}
add_filter('excerpt_length', 'sib_excerpt_length', 100);
...
function sib_page_nav( $html_id ) {
	...
	if ( $wp_query->max_num_pages > 1 ) : ?>
		<nav id="<?php echo esc_attr( $html_id ); ?>" class="navigation" role="navigation">
			<span class="previous"><?php next_posts_link( '<span class="meta-nav">&larr;</span> 早期文章' ); ?></span>
			<span class="next"><?php previous_posts_link( '较新文章 <span class="meta-nav">&rarr;</span>' ); ?></span>
		</nav>
	<?php endif;
}
endif;

if ( ! function_exists( 'sib_comment' ) ) :
function sib_comment( $comment, $args, $depth ) {
	...
	<li <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
		<article id="comment-<?php comment_ID(); ?>" class="comment">
			...
			<?php if ( '0' == $comment->comment_approved ) : ?>
				<p class="waiting-moderation">您的评论正在等待审核</p>
			<?php endif; ?>
